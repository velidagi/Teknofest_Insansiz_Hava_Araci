...
def alcal():
    goto_position_target_relative_ned(0, 0,3)
    condition_yaw(0, relative=True)
		...
		if circles is not None:
			...
			if ((100 > orgin_x - circle_x > 0) and (100 > circle_y - orgin_y > 0)):
				...
				if (z == 10):
					...
					print(str(int((abs(orgin_x - ortalamax) * ortalamapx))) + " CM Sol ve " + str(
						abs((orgin_y - ortalamay) * ortalamapx)) + " CM Geriye gidiliyor")
					if((abs(orgin_x - ortalamax) * ortalamapx) >6):
						sol(int((abs(orgin_x - ortalamax) * ortalamapx)))
					if((abs((orgin_y - ortalamay) * ortalamapx))>6):
						geri(int(abs((orgin_y - ortalamay) * ortalamapx)))
				...
				if (z == 10):
					...
					print(str(ortalamax) + " " + str(ortalamay))
					print(str(int((abs(orgin_x - ortalamax) * ortalamapx))) + "CM Sol ve " + str(
						int(abs((orgin_y - ortalamay) * ortalamapx))) + "CM ileri git")
					if((abs(orgin_x - ortalamax) * ortalamapx) >6):
						sol(int((abs(orgin_x - ortalamax) * ortalamapx)))
					if((abs((orgin_y - ortalamay) * ortalamapx))>6):
						ileri(int(abs((orgin_y - ortalamay) * ortalamapx)))
				...
				if (z == 10):
					ortalamax, ortalamay, ortalamapx = ort(dizix, diziy, px_mt_list)
					print(str(ortalamax) + " " + str(ortalamay))
					print(str(int((abs(orgin_x - ortalamax) * ortalamapx))) + "CM sag ve " + str(
						abs((orgin_y - ortalamay) * ortalamapx)) + "CM geriye git")
					if((abs(orgin_x - ortalamax) * ortalamapx) >6):	
						sag(int((abs(orgin_x - ortalamax) * ortalamapx)))
					if((abs((orgin_y - ortalamay) * ortalamapx))>6):
						geri(int(abs((orgin_y - ortalamay) * ortalamapx)))
